...
支持表中现有的 YouTube、Instagram、TikTok 和 X；`Twitter`、`Twitter/X`、`X/Twitter` 均按 X 处理。可以通过“首次登录 / 更新登录状态”手动登录一次并复用本机会话；程序不保存账号密码，也不绕过验证码。输入文件不会被修改。
...
窗口中的运行日志会实时显示当前处理的账号；点击“停止”会在当前账号处理完成后保存已完成的结果。如果所选输出文件已经存在，程序会从该结果文件续跑，并自动跳过已有“采集状态”或“采集时间”的行。
...
发生中断时，输出文件中已有结果会保留。再次使用相同的输入和输出路径即可自动续跑并跳过已读取行，也可以使用 `--start-row` 指定起始行：

```powershell
python crawl_excel.py "KOL名单.xlsx" -o "KOL名单_邮箱采集结果.xlsx" --start-row 120
```
